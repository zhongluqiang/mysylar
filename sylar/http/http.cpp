...
static const char *s_methods_string[] = {
...
const char *HttpMethodToString(const HttpMethod &m) {
    ...
    if (idx >= sizeof(s_methods_string) / sizeof(s_methods_string[0])) {
        return "<unknown>";
    }
    return s_methods_string[idx];
}
...
std::ostream &HttpResponse::dump(std::ostream &os) const {
    os << "HTTP/" << ((uint32_t)(m_version >> 4)) << "."
       << ((uint32_t)(m_version & 0xf)) << " " << (uint32_t)m_status << " "
...
std::ostream &operator<<(std::ostream &os, const HttpResponse &rep) {
    return rep.dump(os);
}
