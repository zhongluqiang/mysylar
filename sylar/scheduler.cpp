...
void Scheduler::tickle() { SYLAR_LOG_INFO(g_logger) << "ticlke"; }
...
void Scheduler::run() {
    ...
    ScheduleTask task;
    while (true) {
        // SYLAR_LOG_DEBUG(g_logger) << "while(true)";
        task.reset();
        ...
        if (task.fiber) {
            ...
            if (idle_fiber->getState() == Fiber::TERM) {
                // 如果调度器没有调度任务，那么idle协程一直resume/yield，不会结束，如果idle_fiber结束了，那一定是调度器停止了
